...
class TestBatchCommand(unittest.TestCase):
    ...
    def test_command_creation(self):
        ...
        command = BatchCommand(
            natural_language="find large files", line_number=1, timeout=30
        )
    ...
    def test_command_with_dependencies(self):
        ...
        command = BatchCommand(
            natural_language="copy files", line_number=2, depends_on=[1]
        )
...
class TestBatchExecutor(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.executor = BatchExecutor(
            self.session_context, self.tool_registry, self.llm
        )
    ...
    def test_dependencies_satisfied(self):
        ...
        satisfied = self.executor._dependencies_satisfied(
            command_missing_dep, completed_groups
        )
    ...
    def test_should_execute_conditions(self):
        ...
        self.executor.results = [
            BatchResult(command=BatchCommand("prev"), success=True)
        ]
        ...
        self.executor.results = [
            BatchResult(command=BatchCommand("prev"), success=False)
        ]
        command = BatchCommand("test", condition="success")
...
"""
    ...
    def test_format_results(self):
        ...
        results = [
            BatchResult(
                command=BatchCommand(
                    "find files", generated_command="find . -name '*.txt'"
                ),
                success=True,
                output="file1.txt\nfile2.txt",
                duration=1.2,
