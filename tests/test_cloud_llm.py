...
from nlcli.cloud_llm import (
    CloudLLMConfig, CloudLLMProvider, CloudLLMService, CloudLLMResponse,
    create_cloud_llm_service, get_cloud_llm_service
)
...
class TestCloudLLMProvider(unittest.TestCase):
    ...
    def test_http_error(self, mock_urlopen):
        ...
        mock_error.read = Mock(return_value=json.dumps({
...
class TestCloudLLMService(unittest.TestCase):
    ...
    def test_fallback_on_failure(self, mock_make_request):
        ...
        mock_make_request.side_effect = [
            CloudLLMResponse(text="", provider="openai", model="gpt-3.5-turbo", success=False, error="API error"),
            CloudLLMResponse(text="Fallback response", provider="anthropic", model="claude-3", success=True)
        ]
    ...
    def test_all_providers_fail(self, mock_make_request):
        ...
        mock_make_request.return_value = CloudLLMResponse(
            text="", provider="openai", model="gpt-3.5-turbo", 
    ...
    def test_generate_with_system_prompt(self):
        ...
        with patch.object(self.service.provider, 'make_request') as mock_request:
            mock_request.return_value = CloudLLMResponse(
                text="Response with system", provider="openai", 
...
class TestGlobalFunctions(unittest.TestCase):
    ...
    @patch.dict('os.environ', {'NLCLI_CLOUD_LLM_ENABLED': 'true', 'NLCLI_CLOUD_LLM_PROVIDER': 'anthropic'})
