...
class TestPhase2Tools(unittest.TestCase):
    ...
    def test_process_tools_loaded(self):
        ...
        expected_tools = ["list_processes", "kill_process", "process_by_port", "process_tree", "system_resources"]
    ...
    def test_network_tools_loaded(self):
        ...
        expected_tools = ["ping_host", "http_request", "network_connections", "dns_lookup", "download_file", "network_interfaces"]
    ...
    def test_package_tools_loaded(self):
        ...
        expected_tools = ["brew_search", "brew_info", "brew_list", "apt_search", "apt_info", "apt_list"]
    ...
    def test_git_tools_loaded(self):
        ...
        expected_tools = ["git_status", "git_log", "git_diff", "git_branch", "git_show", "git_remote", "git_blame"]
    ...
    def test_danger_levels(self):
        ...
        safe_tools = ["list_processes", "ping_host", "git_status", "brew_search", "apt_search"]
    ...
    def test_tool_examples(self):
        ...
        all_tools = (get_process_tools() + get_network_tools() + 
                    get_package_tools() + get_git_tools())
        
        for tool in all_tools:
            self.assertTrue(len(tool.examples) > 0, f"Tool {tool.name} should have examples")
            for example in tool.examples:
                self.assertIn("nl", example, f"Example for {tool.name} should have 'nl' key")
                self.assertIn("args", example, f"Example for {tool.name} should have 'args' key")
    ...
    def test_tool_keywords(self):
        ...
        for tool in all_tools:
            self.assertTrue(len(tool.keywords) > 0, f"Tool {tool.name} should have keywords")
            self.assertIsInstance(tool.keywords, list, f"Tool {tool.name} keywords should be a list")
