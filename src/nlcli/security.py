    ...
    def __post_init__(self):
        if self.allowed_file_extensions is None:
            self.allowed_file_extensions = {
                '.txt', '.log', '.json', '.yaml', '.yml', '.md', '.csv', '.tsv'
            }
        if self.blocked_directories is None:
            self.blocked_directories = {
                '/etc/shadow', '/etc/passwd', '/root', '/boot', '/sys', '/proc/sys'
            }
...
class SecurityAuditor:
    ...
    def __init__(self, policy: Optional[SecurityPolicy] = None):
        ...
        self.vulnerability_patterns = {
            VulnerabilityType.COMMAND_INJECTION: [
                (r';\s*rm\s+-rf', SecurityLevel.CRITICAL, "Command chaining with destructive rm"),
            ...
            VulnerabilityType.PRIVILEGE_ESCALATION: [
                ...
                (r'chmod\s+[4567]\d\d', SecurityLevel.HIGH, "Setuid/setgid permissions"),
            ],
            VulnerabilityType.DATA_EXFILTRATION: [
    ...
    def audit_command(self, intent: Intent, context: SessionContext) -> List[SecurityViolation]:
        """
    ...
    def _get_recommendation(self, vuln_type: VulnerabilityType) -> str:
        ...
        return recommendations.get(vuln_type, "Review command for security implications")
    
    def get_security_report(self) -> Dict:
        """Generate security audit report."""
        if not self.violation_history:
            return {"status": "clean", "violations": 0, "summary": "No security violations detected"}
...
def audit_command_security(intent: Intent, context: SessionContext) -> List[SecurityViolation]:
