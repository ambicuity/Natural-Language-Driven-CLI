...
class CloudLLMConfig:
    ...
    def __post_init__(self):
        ...
        if not self.enabled and (self.openai_api_key or self.anthropic_api_key or self.google_api_key):
            self.enabled = True
...
class CloudLLMProvider:
    ...
    def make_request(self, messages: List[Dict[str, str]], provider: str) -> CloudLLMResponse:
    ...
    def _request_openai(self, messages: List[Dict[str, str]], start_time: float) -> CloudLLMResponse:
        ...
        return self._make_http_request(url, headers, data, "openai", self.config.openai_model, start_time)
    
    def _request_anthropic(self, messages: List[Dict[str, str]], start_time: float) -> CloudLLMResponse:
        ...
        return self._make_http_request(url, headers, data, "anthropic", self.config.anthropic_model, start_time)
    
    def _request_google(self, messages: List[Dict[str, str]], start_time: float) -> CloudLLMResponse:
        ...
        contents = []
        for msg in messages:
            if msg["role"] != "system":  # Gemini doesn't use system messages in the same way
                contents.append({
        ...
        return self._make_http_request(url, headers, data, "google", self.config.google_model, start_time)
    
    def _make_http_request(self, url: str, headers: Dict[str, str], data: Dict[str, Any], 
                          provider: str, model: str, start_time: float) -> CloudLLMResponse:
    ...
    def _extract_tokens_used(self, response_data: Dict[str, Any], provider: str) -> int:
        ...
        try:
            if provider == "openai":
                ...
                return response_data.get("usage", {}).get("input_tokens", 0) + response_data.get("usage", {}).get("output_tokens", 0)
            elif provider == "google":
...
class CloudLLMService:
    ...
    def is_available(self) -> bool:
        ...
        return self.config.enabled and (
            ...
            self.config.anthropic_api_key or 
            self.config.google_api_key
        )
    
    def generate_response(self, prompt: str, system_prompt: Optional[str] = None) -> CloudLLMResponse:
        ...
        providers_to_try = [self.config.primary_provider] + [
            p for p in self.config.fallback_providers if p != self.config.primary_provider
        ]
        ...
        for provider in providers_to_try:
            ...
            for attempt in range(self.config.max_retries + 1):
                ...
                if response.success:
                    self.logger.info(f"Cloud LLM request successful with {provider} (attempt {attempt + 1})")
                    return response
                
                last_response = response
                self.logger.warning(f"Cloud LLM request failed with {provider} (attempt {attempt + 1}): {response.error}")
