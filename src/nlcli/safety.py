...
class SafetyGuard:
    ...
    def _are_paths_safe(self, intent: Intent, context: SessionContext) -> bool:
        ...
        if not intent.command.startswith("curl") and not intent.command.startswith(
            "wget"
        ):
...
def enhanced_safety_check(
    ...
    try:
        ...
        message = (
            "Command appears safe" if is_safe else "Command blocked by safety checks"
        )
        violations = []
        ...
        if security_violations:
            ...
            critical_violations = [
                v for v in security_violations if v.severity.value == "critical"
            ]
            if critical_violations:
                is_safe = False
                message = (
                    f"Critical security violations detected: {len(critical_violations)}"
                )

        # Enterprise policy check (if enabled)
        enterprise_manager = get_enterprise_manager()
        if enterprise_manager.config_manager.get_config_value(
