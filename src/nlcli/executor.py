...
from nlcli.context import SessionContext, ExecutionResult
...
class CommandExecutor:
    ...
    def execute(self, command: str, context: SessionContext, timeout: Optional[int] = None) -> ExecutionResult:
        """
    ...
    def _dry_run_find(self, command: str, context: SessionContext) -> str:
        ...
        for i, part in enumerate(parts):
            if part not in ["-type", "-size", "-mtime", "-name", "-print0"] and not part.startswith("-") and i > 0:
                path = part
                break
        ...
        condition_str = " and ".join(conditions) if conditions else "no specific conditions"
...
def execute(command: str, context: SessionContext, timeout: Optional[int] = None) -> ExecutionResult:
    """
    Execute a command.
