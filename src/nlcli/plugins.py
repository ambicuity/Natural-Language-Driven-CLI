...
class PluginManager:
    ...
    def load_plugin(self, plugin_path: Path) -> Optional[LoadedPlugin]:
        ...
        try:
            ...
            if not hasattr(module, 'get_tools') or not hasattr(module, 'get_plugin_info'):
            ...
            if not isinstance(tools, list):
                self.logger.error(f"Plugin {plugin_path} get_tools() must return a list")
                return None
            ...
            for tool in tools:
                if isinstance(tool, ToolSchema):
                    ...
                    self.logger.warning(f"Invalid tool in plugin {metadata.name}: {tool}")
